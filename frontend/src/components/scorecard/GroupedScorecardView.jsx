    ...
    return (
      ...
      >
        ...
        {periods.map((periodDate, periodIndex) => {
          ...
          // Debug ONLY for Collection Calls metric
          if (metric.name === 'Collection Calls' && periodIndex === 0) {
            console.log('🔍 Collection Calls date check:', {
              periodDate,
              ...
              <th key={dates[index]} className={`text-center p-2 font-semibold text-xs w-28 ${
                ...
                <div className="flex flex-col items-center">
                  <span className="text-xs font-normal text-gray-500 mb-1">
                    {isCurrentPeriod && type === 'weekly' ? 'Last Week' : ''}
                  </span>
                  <span>{label}</span>
                </div>
              </th>
