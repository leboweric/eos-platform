...
export const completionTrackingService = {
  ...
  async getCompletionStates(orgId) {
    const response = await axios.get(`/organizations/${orgId}/completion-states`);
    return response.data;
  },

  // Toggle 3-year picture item completion
  async toggleThreeYearItem(orgId, itemIndex) {
    const response = await axios.put(`/organizations/${orgId}/three-year-items/${itemIndex}/toggle`);
    return response.data;
  },

  // Toggle 1-year plan goal completion
  async toggleOneYearGoal(orgId, goalIndex) {
    const response = await axios.put(`/organizations/${orgId}/one-year-goals/${goalIndex}/toggle`);
    return response.data;
  }
};