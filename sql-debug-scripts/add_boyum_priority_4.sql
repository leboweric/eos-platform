-- =====================================================
-- Add Fourth Q3 2025 Priority with Milestones for Boyum
-- Priority: Wealth Management Integration
-- =====================================================
...
BEGIN
    ...
    WHERE organization_id = org_id
    AND title = 'Wealth Management Integration'
    AND quarter = 'Q3'
    AND year = 2025;
    ...
    ELSE
        ...
        INSERT INTO quarterly_priorities (
            ...
            'Define processes and relationships for wealth management service integration',
    ...
    -- Now add the milestones (all due Aug 20)
    INSERT INTO priority_milestones (id, priority_id, title, due_date, completed, created_at, updated_at)
    VALUES 
        (gen_random_uuid(), p_id, 'Define process for moving clients back and forth', '2025-08-20'::DATE, false, NOW(), NOW()),
        (gen_random_uuid(), p_id, 'Define how we work together on tax clients', '2025-08-20'::DATE, false, NOW(), NOW()),
        (gen_random_uuid(), p_id, 'Define process for prospective clients either way', '2025-08-20'::DATE, false, NOW(), NOW()),
        (gen_random_uuid(), p_id, 'Define bill manager relationship', '2025-08-20'::DATE, false, NOW(), NOW());
    
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Priority and Milestones Added Successfully!';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Priority ID: %', p_id;
    RAISE NOTICE 'Title: Wealth Management Integration';
    RAISE NOTICE 'Quarter: Q3 2025';
    RAISE NOTICE 'Milestones: 4 (all due Aug 20)';
    RAISE NOTICE '========================================';
...
AND p.title = 'Wealth Management Integration'
...
-- Show FINAL SUMMARY of all Boyum Q3 2025 Company Priorities
SELECT 
    'FINAL SUMMARY' as type,
    ROW_NUMBER() OVER (ORDER BY p.created_at) as priority_number,
    LEFT(p.title, 60) as title,
    COUNT(pm.id) as milestone_count,
    MIN(pm.due_date) as first_milestone,
    MAX(pm.due_date) as last_milestone
FROM quarterly_priorities p
LEFT JOIN priority_milestones pm ON pm.priority_id = p.id
WHERE p.organization_id = (SELECT id FROM organizations WHERE slug = 'boyum-barenscheer')
AND p.quarter = 'Q3'
AND p.year = 2025
AND p.is_company_priority = true
...
SELECT 
    'TOTALS' as type,
    COUNT(DISTINCT p.id) as total_priorities,
