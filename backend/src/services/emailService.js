      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${data.totalRocks > 0 ? `
          <div style="background-color: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
            <h3 style="color: #1e40af; margin: 0 0 10px 0; font-size: 18px;">🎯 Rock Completion</h3>
            <div style="font-size: 24px; font-weight: bold; color: ${data.rockCompletionPercentage >= 70 ? '#10b981' : data.rockCompletionPercentage >= 40 ? '#f59e0b' : '#ef4444'};">
              ${data.rockCompletionPercentage}%
            </div>
            <div style="color: #666; font-size: 14px; margin-top: 5px;">
              ${data.completedRocks} of ${data.totalRocks} rocks complete
            </div>
          </div>
        ` : ''}
        ...
        ${data.cascadingMessages && data.cascadingMessages.length > 0 ? `
          ...
          <div style="background-color: #faf5ff; padding: 20px; border-radius: 8px; margin: 10px 0; border-left: 4px solid #7c3aed;">
            ${data.cascadingMessages.map(msg => `
              <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #e9d5ff; last-child:border-bottom: none;">
                <p style="color: #333; margin: 0 0 8px 0; line-height: 1.6;">${msg.message}</p>
                <p style="color: #666; font-size: 14px; margin: 0;">— ${msg.createdBy}</p>
              </div>
            `).join('')}
          </div>
        ` : ''}
